...
class zib_shop
{
    ...
    public function comments_custom_column($column, $comment_id)
    {
        switch ($column) {
            case 'response_2':
                //（8.7 行为）static 缓存 + remove_filter，避免 WP_Comments_List_Table::__construct
                //自动注册的 floated_admin_avatar 在每行都 add 一次，导致作者列头像累加
                static $wp_list_table;
                if (!$wp_list_table) {
                    $wp_list_table = new WP_Comments_List_Table();
                    remove_filter('comment_author', array($wp_list_table, 'floated_admin_avatar'), 10);
                }

 ...
 * @LastEditTime : 2026-04-25 10:48:46
...
function zib_get_geographical_position_by_pconline($ip, $debug = false)
{
    ...
    //通过太平洋接口获取
    //8.8 官方原版用 http:// + 正则解析，pconline 现在强制 HTTPS 重定向 + 响应格式变化导致正则全部抓空。
    //回退为 8.7 风格：HTTPS + json_decode + 健壮的空值判断。
    ...
    $addr = !empty($body_data['addr']) ? preg_split('/\s+/u', trim($body_data['addr'])) : array();

    $data = array(
        'ip'       => $ip,
        'nation'   => !empty($addr[0]) ? $addr[0] : '',
        'province' => !empty($body_data['pro']) ? $body_data['pro'] : '',
        'city'     => !empty($body_data['city']) ? $body_data['city'] : '',
        'district' => !empty($body_data['region']) ? $body_data['region'] : '',
        'sdk'      => 'pconline',
    );

    if (empty($data['nation']) && empty($data['province']) && empty($data['city']) && empty($data['district'])) {
        if ($debug) {
            $body_data['message'] = '接口未返回有效地理位置信息';
            return $body_data;
        }
        return false;
    }
...
function zib_fun_throttle($time, ...$args)
{
    ...
    if ($lockHandle && flock($lockHandle, LOCK_EX)) {
        ...
        if (@file_exists($data_file)) {
            $file_fread = @file_get_contents($data_file);
        }

        $file_fread    = $file_fread ? $file_fread : '{}';
        ...
        if (!$write_result) {
            return new WP_Error('file_write_error', '文件夹"' . $temp_dir . '/"写入错误, 请检查文件权限及所有者！');
        }
...
//将字符串按字拆分成数组，适配各种语言编码
function zib_str_split($str, $charset = 'UTF-8')
{
    $str = mb_str_split($str, 1, $charset);
    return $str;
}

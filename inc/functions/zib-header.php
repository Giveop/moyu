...
function zib_menu_button($layout = 1)
{
    ...
    // ===== SFW/NSFW 切换按钮（PC端）=====
    $sfw_but = '';
    if (function_exists('zib_get_sfw_button_html')) {
        $sfw_html = zib_get_sfw_button_html();
        if ($sfw_html) {
            $sfw_but = '<li class="relative sfw-nav-btn">' . $sfw_html . '</li>';
        }
    }
    // ===================================
...
// ===== 移动端顶部导航栏 SFW 按钮（新增）=====
add_action('wp_footer', 'zib_mobile_header_sfw_button', 99);
function zib_mobile_header_sfw_button() {
    if (!function_exists('zib_get_sfw_button_html')) return;
    ...
    echo '<style>
        @media (min-width: 768px) {
            .mobile-header-sfw-btn { display: none !important; }
        }
        .mobile-header-sfw-btn {
            position: fixed;
            top: 0;
            right: 50px;
            height: 50px;
        ...
        .mobile-header-sfw-btn .sfw-filter-box {
            height: 100%;
            display: flex;
            align-items: center;
        }
        .mobile-header-sfw-btn .dropdown-toggle {
            padding: 0 8px;
            color: inherit;
        ...
        .mobile-header-sfw-btn .fa-shield {
            font-size: 1.2em;
        }
        .mobile-header-sfw-btn .sfw-text {
            font-size: 0.8em;
            margin-left: 3px;
        }
        .mobile-header-sfw-btn .dropdown-menu {
            position: fixed;
            top: 50px;
            right: 10px;
            left: auto;
    ...
    echo '<div class="mobile-header-sfw-btn visible-xs-block">' . $sfw_html . '</div>';
}
// ===========================================

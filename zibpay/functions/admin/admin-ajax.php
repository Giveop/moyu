 ...
 * @LastEditTime : 2026-04-29 22:39:10
 * @Project      : Zibll子比主题
...
//获取用户排行数据
//注意：8.8 官方原版用 ZibDB 链式 + metaQuery + ORDER BY 引用别名，
//在 8.7 兼容层（db-class-legacy.php）中会生成 SQL 错误（Unknown column '_mt.meta_value' in 'order clause'）。
//回退为 8.7 的原生 wpdb prepare 实现，等价但更可靠。
function zibpay_get_user_ranking_data($type = 'points')
{
    global $wpdb;

    $meta_key = $type == 'balance' ? 'balance' : 'points';
    $limit    = 50;

    $sql = $wpdb->prepare(
        "SELECT MAX(um.meta_value+0) AS price, u.display_name AS display_name, u.ID AS user_id
        FROM {$wpdb->users} AS u
        INNER JOIN {$wpdb->usermeta} AS um
            ON u.ID = um.user_id
            AND um.meta_key = %s
        WHERE um.meta_value+0 > 0

...
if (!empty($_GET['delete'])) {
    ...
    if (!isset($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], 'admin_order_delete')) {
        echo '<div class="updated notice-alt"><h4 style="color:rgb(242, 123, 94);">链接已过期，请刷新页面后稍候再试</h4><p><a href="' . $order_url . '" class="button button-primary">返回订单中心</a></p></div>';
        exit;
    }
...
if ($list) {
    ...
    foreach ($list as $value) {
        ...
        $edit = '<a class="" onclick="return confirm(\'确认删除此订单?  删除后数据不可恢复!\')" href="' . $order_url . '&delete=' . $value->id . '&_wpnonce=' . wp_create_nonce('admin_order_delete') . '">删除</a>';
        ...
        $order_type_colors = ['#ff4747', '#ee5307', '#1e8608', '#1a8a65', '#0c9cc8', '#086ae8', '#3353fd', '#4641e8', '#853bf2', '#e94df7', '#ca2b7d', '#d7354c', '#ff4747', '#8e24ac'];
        $order_type_index  = abs((int) $value->order_type);
        $order_type_color  = isset($order_type_colors[$order_type_index]) ? $order_type_colors[$order_type_index] : $order_type_colors[$order_type_index % count($order_type_colors)];
        $order_type_link   = '<a style="color: ' . $order_type_color . ';" href="' . $order_url . '&order_type=' . $value->order_type . '">' . $order_type . '</a>';
        $user_a          = '未登录用户';
        if ($value->user_id) {
